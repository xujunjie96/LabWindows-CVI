...
static unsigned int conversationHandle2;//TCP连接句柄  
					   ...
					   void *callbackData, int eventData1, int eventData2)
{
	...
	switch (event)
	{
		case EVENT_TIMER_TICK:
			...
			if(conversationHandle2)
			{
				 datapointernumber[0]=result;
				 ServerTCPWrite (conversationHandle2, datapointernumber, 10, 0);
			}
			break;
	}
	return 0;
...
int TCPServerCB(unsigned handle,int xType,int errCode,void *callbackData)
{
	 ...
	 switch(xType)
	 {
		 ...
		 case TCP_CONNECT :
			 ...
			 {
			 	   conversationHandle2=handle; //当有客户机连接时，获得tcp连接句柄
				   i++; 
			 }
			 break;
		//TCP断开时产生的事件 
